...
int blit_load_font()
{
	SceUID fd = sceIoOpen("ms0:/seplugins/font_vshmenu.bin", PSP_O_RDONLY, 0);
	if(fd < 0)
	{
		fd = sceIoOpen("ef0:/seplugins/font_vshmenu.bin", PSP_O_RDONLY, 0);
		if(fd < 0) return fd;
	}

	int read = sceIoRead(fd, msx, sizeof(msx));
