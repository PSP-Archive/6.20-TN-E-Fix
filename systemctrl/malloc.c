...
int mallocinit()
{
	...
	if(keyconfig == PSP_INIT_KEYCONFIG_VSH) size = 14 * 1024;
	else if(keyconfig == PSP_INIT_KEYCONFIG_GAME && sceKernelInitApitype() == 0x123) return 0;
...
{
	*memid = sceKernelAllocPartitionMemory620(partitionid, "", PSP_SMEM_Low, size, NULL);
	if(*memid < 0) return 0;
	return sceKernelGetBlockHeadAddr620(*memid);
}

void tn_free(SceUID memid)
{
	sceKernelFreePartitionMemory620(memid);
}